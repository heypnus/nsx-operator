...
func (r *PodReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if pod.Namespace != "test-exec-ns" {
		log.Info("only handle pod in namespace test-exec-ns", "pod", req.NamespacedName)
		return common.ResultNormal, nil
	}
	...
	if pod.ObjectMeta.DeletionTimestamp.IsZero() {
		...
		if err != nil {
			log.Error(err, "failed to get NSX resource path from subnet", "pod", pod)
			return common.ResultRequeue, err
		}
		log.Info("got NSX subnet for pod", "NSX subnet path", nsxSubnetPath, "pod.Name", pod.Name, "pod.UID", pod.UID)
		contextID, err := common.ServiceMediator.GetNodeIDByName(pod.Spec.NodeName)
		if err != nil {
			log.Error(err, "failed to get node ID for pod", "pod", req.NamespacedName, "node", pod.Spec.NodeName)
			return common.ResultRequeue, err
		}
		nsxSubnetPortState, err := r.Service.CreateOrUpdateSubnetPort(pod, nsxSubnetPath, contextID)
		if err != nil {
			log.Error(err, "failed to create or update NSX subnet port, would retry exponentially", "pod", req.NamespacedName)
			updateFail(r, &ctx, pod, &err)
			return common.ResultRequeue, err
		}
		podAnnotationChanges := map[string]string{
			common.AnnotationPodMAC:        strings.Trim(*nsxSubnetPortState.RealizedBindings[0].Binding.MacAddress, "\""),
			common.AnnotationPodAttachment: *nsxSubnetPortState.Attachment.Id,
		}
		err = util.UpdateK8sResourceAnnotation(r.Client, &ctx, pod, podAnnotationChanges)
		if err != nil {
			log.Error(err, "failed to update pod annotation", "pod", req, "podAnnotationChanges", podAnnotationChanges)
...
func (r *PodReconciler) GetSubnetPathForPod(ctx context.Context, pod *v1.Pod) (string, error) {
	...
	log.Info("allocated Subnet for Pod", "subnetPath", subnetPath, "pod.Name", pod.Name, "pod.UID", pod.UID)
	if err != nil {
		return subnetPath, err
	}
	log.Info("allocated NSX subnet for pod", "nsxSubnetPath", subnetPath, "pod.Name", pod.Name)
