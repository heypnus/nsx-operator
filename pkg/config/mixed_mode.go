...
var (
	...
	// namespaceRefreshReader, when non-nil, is used by RefreshMixedMode to list
...
// networkProviderLabelExistsSelector limits namespace listing to objects that
// carry iaas.vmware.com/network-provider — the only namespaces that can flip
// mixed-mode flags. This shrinks list traffic versus listing every namespace.
func networkProviderLabelExistsSelector() (labels.Selector, error) {
	return metav1.LabelSelectorAsSelector(&metav1.LabelSelector{
		MatchExpressions: []metav1.LabelSelectorRequirement{
			{Key: NetworkProviderLabel, Operator: metav1.LabelSelectorOpExists},
		},
	})
}

func accumulateNetworkProviderFlags(items []v1.Namespace) (hasT1 bool, hasVPC bool) {
	for i := range items {
		provider := items[i].Labels[NetworkProviderLabel]
		switch provider {
		case ProviderNSXT1:
			hasT1 = true
		case ProviderNSXVPC:
...
func scanNamespaceLabels(ctx context.Context, clientset kubernetes.Interface) (hasT1 bool, hasVPC bool, err error) {
	sel, err := networkProviderLabelExistsSelector()
	if err != nil {
		return false, false, err
	}
	nsList, err := clientset.CoreV1().Namespaces().List(ctx, metav1.ListOptions{LabelSelector: sel.String()})
	if err != nil {
		return false, false, err
	}
	hasT1, hasVPC = accumulateNetworkProviderFlags(nsList.Items)
	return hasT1, hasVPC, nil
}

func scanNamespaceLabelsWithClient(ctx context.Context, reader client.Reader) (hasT1 bool, hasVPC bool, err error) {
	sel, err := networkProviderLabelExistsSelector()
	if err != nil {
		return false, false, err
	}
	nsList := &v1.NamespaceList{}
	if err := reader.List(ctx, nsList, client.MatchingLabelsSelector{Selector: sel}); err != nil {
		return false, false, err
	}
	hasT1, hasVPC = accumulateNetworkProviderFlags(nsList.Items)
	return hasT1, hasVPC, nil
...
// RefreshMixedMode keeps using the kubernetes.Interface from InitMixedMode.
...
func RefreshMixedMode(ctx context.Context, clientset kubernetes.Interface) bool {
	...
	if r := currentNamespaceRefreshReader(); r != nil {
		newT1, newVPC, err = scanNamespaceLabelsWithClient(ctx, r)
	} else {
